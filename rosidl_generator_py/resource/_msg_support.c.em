...
@[    if isinstance(member.type.basetype, BasicType) and member.type.basetype.type == 'char']@
      ...
      @primitive_msg_type_to_c(member.type.basetype) tmp = PyUnicode_1BYTE_DATA(item)[0];
...
@[  elif isinstance(member.type, BasicType) and member.type.type == 'char']@
    ...
    ros_message->@(member.name) = PyUnicode_1BYTE_DATA(field)[0];
...
@[  elif isinstance(member.type, String)]@
    assert(PyUnicode_Check(field));
    rosidl_generator_c__String__assign(&ros_message->@(member.name), (char *)PyUnicode_1BYTE_DATA(field));
...
@[  elif isinstance(member.type, String)]@
    field = PyUnicode_FromString(ros_message->@(member.name).data);
@[  elif isinstance(member.type, BasicType) and member.type.type == 'boolean']@
